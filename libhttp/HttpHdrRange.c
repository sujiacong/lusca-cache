...
/*
 * $Id: HttpHdrRange.c 14599 2010-04-14 03:49:57Z adrian.chadd $
 *
...
/* XXX this should be in some config struct, setup via some function call */
squid_off_t cfg_range_offset_limit = -0;
...
httpHdrRangeOffsetLimit(HttpHdrRange * range)
{
	...
	/* not a range request */
	return 0;
    if (!cfg_range_offset_limit)
	/* disabled */
	return 1;
    if (-1 == cfg_range_offset_limit)
	/* forced */
	return 0;
    if (cfg_range_offset_limit >= httpHdrRangeFirstOffset(range))
	/* below the limit */
	return 0;
    return 1;
}
